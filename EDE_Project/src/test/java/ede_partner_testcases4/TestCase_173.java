...
public class TestCase_173 extends BaseTest {
  @Test  public void testCase_173() throws Exception {

		ExtentTestManager.createTest("PTC173", "Partner Test Case 173");
		String tcDataFile = "partner_173.properties";
...
//		String someoneElse = "Someone else who isn't applying for coverage";
		...
		String s012SameNameQuestionMale = "Is " + applicantFullName + " " + Utility.getFromTextProperties("s012SameNameQuestion");
...
//		String s015Question = "Does this match the name and date of birth on "+applicantFullName+"'s Social Security card?";
		...
		String s028Question = "Who is the tax filer that will claim "+applicantFullName+" on their income tax return?";
		String s028QuestionChild = "Who is the tax filer that will claim "+childFullName+" on their income tax return?";
		...
		if(state.trim().equalsIgnoreCase("ms")) {
			s046QuestionMedicad = Utility.getFromTextProperties("s046QuestionMedicadMS") + " " + minus90DaysDate + "?";
		} else if(state.trim().equalsIgnoreCase("ga")) {
			s046QuestionMedicad = Utility.getFromTextProperties("s046QuestionMedicadGA") + " " + minus90DaysDate + "?";
		}
		...
		String s049QuestionMedicad = null, s049QuestionMedicadChild = null;
		if(state.trim().equalsIgnoreCase("ms")) {
			s049QuestionMedicad = "Did " + applicantFullName + " " + Utility.getFromTextProperties("s049QuestionMedicadMS");
			s049QuestionMedicadChild = "Did " + childFullName + " " + Utility.getFromTextProperties("s049QuestionMedicadMS");
		} else if(state.trim().equalsIgnoreCase("ga")) {
			s049QuestionMedicad = "Did " + applicantFullName + " " + Utility.getFromTextProperties("s049QuestionMedicadGA");
			s049QuestionMedicadChild = "Did " + childFullName + " " + Utility.getFromTextProperties("s049QuestionMedicadGA");
		}
...
//		String s081Question2 = "Why did "+childFullName+"'s coverage end? (Optional)";
		...
		String minus05DaysDate2 = ESTTimeZone.getPreviousDate("05", "MM/dd/yyyy");
		String minus95DaysDate2 = ESTTimeZone.getPreviousDate("95", "MM/dd/yyyy");
		...
		String s027QuestionAboutDependents20 = "Who are " + applicantFullName;
...
//		commonMethodPage.selectToAddNonAplicantSpouse_S026("Someone else who isn't applying for coverage");
		...
		commonMethodPage.verifyS027Dependent(s027QuestionDependentSingle, s027DependentDrawerHeading,s027DependentDrawerContent);
		commonMethodPage.selectIsAnyDependent(yes);
		commonMethodPage.verifyQuestionContents("span", s027QuestionAboutDependents20, s027QuestionAboutDependents2, 2, 1);
		commonMethodPage.chooseAnOption(childFullName, 1, 1);
		
		/*
//		commonMethodPage.selectToAddNonAplicantSpouse_S026(someoneApplying);
		commonMethodPage.chooseAnOption("Someone else who isn", 2, 1);
		Wait.wait2Second();
...
//		commonMethodPage.verifyQuestionContents(spanTag, s041AQuestion, s041AQuestion, 2, 1);
		commonMethodPage.chooseAnOption(yes, 1, 1);
		commonMethodPage.clickSaveAndContinueButton();
		commonMethodPage.clickSaveAndContinueButton();
		...
		commonMethodPage.verifyQuestionContents("span", s038Question1, s038Question1, 1, 1);
		commonMethodPage.chooseAnOption(childFullName, 1, 1);
		commonMethodPage.clickSaveAndContinueButton();
		...
		commonMethodPage.verifyQuestionContents("span", s028QuestionChild, s028QuestionChild, 1, 1);
...
//		Wait.wait2Second();
		...
		*/
		...
		// 2nd Appl
		commonMethodPage.verifyQuestionContents(spanTag, s049QuestionMedicadChild, s049QuestionMedicadChild, 1, 1);
...
//		commonMethodPage.enterEducationExpenses(educationExpenses);
		...
		// Income Questions - 2nd Appl - Child
		ExtentTestManager.getTest().info("---------------------SCREEN S057----------------------------");
